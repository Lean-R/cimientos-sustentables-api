...
app.get("/obras/:id", (req, res) => {
  ...
  .map((g) => {
    const partida = partidas.find((p) => Number(p.id) === Number(g.partidaId));
  ...
  const valor = Number(partida.precioTotal.replace(/\$/g, "").replace(/\./g, "").replace(",", "."));
  return acc + valor;
}, 0);

const totalGastos = gastosDeLaObra.reduce((acc, gasto) => {
  const valor = Number(gasto.monto.replace(/\$/g, "").replace(/\./g, "").replace(",", "."));
  return acc + valor;
}, 0);
